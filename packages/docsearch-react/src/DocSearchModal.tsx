...
import type { createAutocomplete, AlgoliaInsightsHit } from '@algolia/autocomplete-core';
...
import { buildAskAiSources, buildRecentConversationsSource } from './sources/askAi';
...
import { identity, noop, isModifierEvent, scrollTo as scrollToUtils } from './utils';
...
export function DocSearchModal({
  ...
  // TODO: Move into `useAutocomplete?`
  ...
  autocompleteRef.current = useAutocomplete({
    ...
    getSources({ query, state: sourcesState, setContext, setStatus }) {
      if (!query) {
        ...
        const recentConversationSource = buildRecentConversationsSource({
          canHandleAskAi,
          conversations,
          disableUserPersonalization,
          onAskAiToggle,
      ...
      // Ask AI source
      const askAiSource = buildAskAiSources({
        query,
