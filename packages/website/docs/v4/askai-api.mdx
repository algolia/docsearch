...
- **Dual SDK support**: Choose between v4 (legacy) and v5 (recommended) response formats
...
The Ask AI API supports two response formats to accommodate different integration needs:

### Version 5 (Recommended)

- **Enhanced streaming format** with improved real-time response handling
- **Better error handling** and status reporting
- **Optimized for modern AI SDKs** like Vercel AI SDK
- **Future-proof** with ongoing improvements and features

### Version 4 (Legacy)

- **Legacy streaming format** for backward compatibility
- **Maintained for existing integrations** that haven't migrated yet
- **Will be deprecated** in future releases

### Requesting SDK Versions

Specify your preferred SDK version using the `X-AI-SDK-Version` header:

```http
X-AI-SDK-Version: v5
```

If no version is specified, the API defaults to **v4** for backward compatibility.

---

## Authentication

Ask AI uses authorization tokens for authentication. Tokens expire after 5 minutes, so you'll need to request a new one before each chat request.
...
- `X-AI-SDK-Version`: `v4` or `v5` (optional, defaults to v4)
...
1. **Token Management**: Always request a fresh token before chat requests
