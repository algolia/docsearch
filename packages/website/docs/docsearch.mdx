...
To enable Ask AI, add your Agent Studio assistant ID and the markdown/content indices the agent can query:
...
- Use `askAi.indices` to describe the content indices the Agent Studio assistant can search.
- Use per-index `searchParameters` to scope the agent to a language, version, or content type.
