...
### Why do I see two Algolia app in my dashboard?

We did not remove access to the legacy DocSearch application (`BH4D9OD16A`) to give you the time to get familiar with our new infra. `BH4D9OD16A` will remain available until the migration has been completed for all the DocSearch users.
