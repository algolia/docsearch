...
Copy the **agent ID**. DocSearch calls this value `assistantId` in its public API.
...
Pass the Agent Studio **agent ID** as `askAi.assistantId`, or as the `askAi` string shorthand. Don't add `agentStudio` or `useStagingEnv`.
...
The adapter requires `themeConfig.docsearch.askAi` as an object with `assistantId`. It rejects the string shorthand and the removed `agentStudio` property. See the [Docusaurus adapter guide](/docs/packages/docusaurus-adapter/getting-started).
