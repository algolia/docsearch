...
const LANDING_COMMANDS: LandingCommand[] = [
  ...
  { args: '<docset-id> <query>', description: 'Search inside specific docsets', name: 'query' },
];
...
export function renderHelp(): string {
  ...
  return [
    ...
    `  ${cmd('query')}   ${color.cyan('<docset-id> <query>')}   Search inside specific docsets`,
    '',
    heading('Options'),
    `  ${color.green('--project')}                     Install setup files into the current project`,
    `  ${color.green('--global')}                      Install setup files globally ${meta('(default)')}`,
    `  ${color.green('--endpoint')} ${color.cyan('<url>')}             Override the hosted DocSearch MCP endpoint`,
    `  ${color.green('--yes')}, ${color.green('-y')}                    Skip the interactive agent prompt`,
