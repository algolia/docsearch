...
describe('buildToolRequest', () => {
  ...
  it('maps resolve to the docset resolver tool', () => {
    const request = buildToolRequest(
      'resolve',
      parseArgs(['Algolia', 'InstantSearch', '--top-n', '4'])
    );
  ...
  it('rejects empty docset IDs', () => {
    expect(() =>
      buildToolRequest('query', parseArgs([',', 'search query']))
    ).toThrow(/docset ID/i);
  });

  it('rejects options that do not belong to the command', () => {
    expect(() =>
      buildToolRequest('resolve', parseArgs(['Next.js', '--max-results', '2']))
    ).toThrow(/not valid for the resolve command/);
  });
  ...
  it('rejects non-HTTP endpoints', () => {
    expect(() =>
      buildToolRequest(
        'resolve',
        parseArgs(['Next.js', '--endpoint', 'file:///tmp/mcp'])
      )
...
describe('buildSetupRequest', () => {
  it('maps setup flags to selected agents and project scope', () => {
    expect(
      buildSetupRequest(
        parseArgs(['--cursor', '--claude', '--project', '--yes'])
      )
    ).toEqual({
  ...
  it('rejects positional setup arguments and unrelated flags', () => {
    expect(() => buildSetupRequest(parseArgs(['cursor']))).toThrow(
      /Unexpected setup argument/
    );
    expect(() => buildSetupRequest(parseArgs(['--json']))).toThrow(
      /not valid for the setup command/
    );
  });
});
