...
var communityHeader = (function() {
  function communityHeader(docSearchCredentials, docSearch) {
    ...
    this.searchContainer = this.searchInputContainer
      ? this.searchInputContainer.parentNode
      : null;
    this.navRoot = document.querySelector('.algc-dropdownroot');
    this.dropdownRoot = document.querySelector(
    ...
    this.subListHolders = []
      .concat(_toConsumableArray(this.subList))
      .map(function(node) {
    ...
    this.hasDocSearchRendered = document.querySelector(
      '.algc-navigation .algc-search__input--docsearch'
    );
  ...
  _createClass(communityHeader, [
    ...
    },
    {
      ...
      value: function shouldInitDocSearch() {
        if (!this.enableDocSearch && this.hasDocSearchRendered) {
          throw new Error(
            'You need to pass docSearch: { apiKey, indexName, inputSelector } to communityHeader function in order to initialise docSearch'
          );
        }
      },
    },
    {
      ...
      value: function checkDocSearch() {
        var docSearch = arguments.length > 0 && arguments[0] !== undefined
          ? arguments[0]
          : false;
        ...
        if (
          typeof window.docsearch === 'function' ||
    ...
    },
    {
      ...
      value: function verifyDocSearchParams(docSearchCredentials) {
        return docSearchCredentials &&
          ...
          docSearchCredentials.inputSelector
          ? true
          : false;
      },
    },
    {
      ...
      value: function triggerMenu(event) {
        ...
        this.dropDownContainer.style.cssText = '\n      transform: translateX(' +
          leftDistance +
          ');\n      width: ' +
          newTargetCoordinates.realWidth +
          'px;\n      height: ' +
          (newTargetCoordinates.realHeight + 10) +
          'px;';
    ...
    },
    {
      ...
      value: function docSearchToggling() {
        ...
        this.searchInput = document.querySelector(
          this.docSearchCredentials.inputSelector
        );
    ...
    },
    {
      ...
      value: function bindListeners() {
        ...
        document
          .querySelector('.algc-dropdownroot__dropdowncontainer')
          .addEventListener('mouseleave', this.closeMenu);
      },
    },
  ]);
